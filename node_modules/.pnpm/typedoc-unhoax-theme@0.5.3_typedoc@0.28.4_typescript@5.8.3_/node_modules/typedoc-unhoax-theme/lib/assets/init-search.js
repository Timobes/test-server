...
const kindById = Object.fromEntries(Object.entries(ReflectionKind).map(([k, v]) => [v, k]));
export async function initSearch() {
    ...
    inputElement.oninput = () => {
        ...
        timeout = setTimeout(async () => {
            ...
            const children = listItems === undefined
                ? resultsPlaceholder
                : listItems.length
...
async function searchResults(query) {
    if (!indexPromise)
        return console.info('index promise is empty'), [];
    query = query.trim();
    if (!query)
        return undefined;
    ...
    return results.map((item) => {
        ...
        nameElement.innerHTML = row.name.replace(new RegExp(`(${query})`, 'gi'), '<b>$1</b>');
        ...
        const option = h.a({ class: 'search-result nav-leaf', href: rowUrl }, [
            ...
            h.small({}, h.small({}, kindName ? `(${pascalCaseToSpaceCase(kindName)})` : 'N/A')),
        ]);
        return option;
    });
}
